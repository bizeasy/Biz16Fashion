...
public class PayPalServices {
    ...
    public static Map<String, Object> setExpressCheckout(DispatchContext dctx, Map<String, ? extends Object> context) {
        ...
        if (!cart.shippingApplies()) {
            ...
            //These are used in conjunction with the Shipping Call back URL.
...
//            encoder.add("L_SHIPPINGOPTIONAMOUNT0", "0.00");
            ...
            {
                encoder.add("NOSHIPPING", "1");
                encoder.add("PAYMENTREQUEST_0_SHIPTOSTREET", shippingAddress.getString("address1"));
                encoder.add("PAYMENTREQUEST_0_SHIPTOSTREET2", shippingAddress.getString("address2"));
                encoder.add("PAYMENTREQUEST_0_SHIPTOCITY", shippingAddress.getString("city"));
                encoder.add("PAYMENTREQUEST_0_SHIPTOSTATE", shippingAddress.getString("stateProvinceGeoId"));
                encoder.add("PAYMENTREQUEST_0_SHIPTOZIP", shippingAddress.getString("postalCode"));
                encoder.add("PAYMENTREQUEST_0_SHIPTOCOUNTRY", shippingAddress.getString("countryGeoId"));
            }
        }
    ...
    private static void addCartDetails(NVPEncoder encoder, ShoppingCart cart) throws GenericEntityException {
        ...
        if (cart.getPaymentTotal().compareTo(BigDecimal.ZERO) != 0) {
            encoder.add("L_NUMBER" + line, "N/A");
            encoder.add("L_NAME" + line, "Applied Payment");
            encoder.add("L_AMT" + line, cart.getPaymentTotal().negate().setScale(2, BigDecimal.ROUND_HALF_UP).toPlainString());
            encoder.add("L_QTY" + line, "1");
            line++;
        }
        encoder.add("ITEMAMT", cart.getSubTotal().add(otherAdjustments).subtract(cart.getPaymentTotal()).setScale(2).toPlainString());
        ...
        encoder.add("AMT", cart.getGrandTotal().subtract(cart.getPaymentTotal()).setScale(2).toPlainString());
...
//        encoder.add("MAXAMT", cart.getSubTotal().add(otherAdjustments).setScale(2).toPlainString());
    ...
    public static Map<String, Object> getExpressCheckout(DispatchContext dctx, Map<String, Object> context) {
        ...
        Debug.log("gotParty" + partyId, module);
        ...
        Debug.log("gotEmail" + emailAddress, module);
        ...
        Debug.log("gotEmailContactMech" + emailContactMechId, module);
        // Phone number
        ...
        Debug.log("gotPhone" + phoneNumber, module);

        //We do not need to create the paypal address.
        /*
        ...
        Debug.log("gotBillingAddr" + postalContactId, module);
        ...
        Debug.log("createPurpose", module);
        
        if (postalContactId == null) {
            postalMap.put("userLogin", cart.getUserLogin());
            try {
                outMap = dispatcher.runSync("createPostalAddress", postalMap);
                postalContactId = (String) outMap.get("contactMechId");
            } catch (GenericServiceException e) {
                Debug.logError(e, module);
                return ServiceUtil.returnError(e.getMessage());
        ...
        if (UtilValidate.isNotEmpty(shipMethod))
        {
            if ("Calculated Offline".equals(shipMethod)) {
                ...
                Debug.log("gotCarrier" + shipMethodSplit[0], module);
                String shippingMethodTypeDesc = StringUtils.join(shipMethodSplit, " - ", 1, shipMethodSplit.length);
                Debug.log("gotShipDesc" + shipMethodSplit, module);
                try {
                    ...
                    Debug.log("gotshipmentMethodTypeID" + shipmentMethod.getString("shipmentMethodTypeId"), module);
        ...
        Debug.log("gotPayPalPayment" + paymentMethodId, module);
    ...
    public static Map<String, Object> doExpressCheckout(DispatchContext dctx, Map<String, Object> context) {
        ...
        BigDecimal subTotal = processAmount.subtract(shippingTotal).subtract(taxTotal).setScale(2, BigDecimal.ROUND_HALF_UP);
