...
rem ###VM args block ####################################################
rem set MEMIF=-Xms@Xms@ -Xmx@Xmx@ -XX:MaxPermSize=@MaxPermSize@
rem # RMI settings
...
rem "%JAVA_HOME%\bin\java" -Xms@Xms@ -Xmx@Xmx@ -XX:MaxPermSize=@MaxPermSize@ -Xdebug -Xnoagent -Djava.compiler=NONE -Xrunjdwp:transport=dt_socket,server=y,suspend=n,address=5005 -jar ofbiz.jar > runtime\logs\console.log
...
rem "%JAVA_HOME%\bin\java" -Xms@Xms@ -Xmx@Xmx@ -XX:MaxPermSize=@MaxPermSize@ -jar ofbiz.jar > runtime\logs\console.log
