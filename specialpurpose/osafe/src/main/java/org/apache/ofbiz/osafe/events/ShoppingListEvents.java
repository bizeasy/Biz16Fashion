...
public class ShoppingListEvents 
{
    ...
    public static void fillAutoSaveList(ShoppingCart cart, LocalDispatcher dispatcher, HttpServletRequest request) throws GeneralException 
    {
        if ( UtilValidate.isNotEmpty(cart) && UtilValidate.isNotEmpty(dispatcher)) 
        {
            ...
            try 
            {
				...
				{
                    org.apache.ofbiz.order.shoppinglist.ShoppingListEvents.addBulkFromCart(delegator, dispatcher, cart, userLogin, autoSaveListId, null, itemsArray, false, false);
                }
            } 
           ...
           try 
           {
               ...
               if (UtilValidate.isNotEmpty(adminLogin))
               {
            	   ...
            	   else
            	   {
		               ...
		               if (UtilValidate.isNotEmpty(itemsArray) && itemsArray.length > 0) 
						{
		                   org.apache.ofbiz.order.shoppinglist.ShoppingListEvents.addBulkFromCart(delegator, dispatcher, cart, adminLogin, autoSaveListId, null, itemsArray, false, false);
		                }
            	   }
               }
          }
        ...
        if (okayToLoad) 
        {
            ...
            try 
            {
               ...
               //addListToCart(delegator, dispatcher, cart, prodCatalogId, autoSaveListId, false, false, false);
                org.apache.ofbiz.order.shoppinglist.ShoppingListEvents.addListToCart(delegator, dispatcher, cart, prodCatalogId, autoSaveListId, false, false, userLogin != null ? true : false);
               	...
               	for (Iterator<?> item = cart.iterator(); item.hasNext();) 
                {
                	...
                	org.apache.ofbiz.osafe.events.ShoppingCartEvents.setProductFeaturesOnCart(cart,cartItem.getProductId());
                }
    ...
    public static String updateAutoSaveListParty(HttpServletRequest request, HttpServletResponse response) {
        ...
        try 
        {
            ...
            {
                ShoppingCart sc = org.apache.ofbiz.order.shoppingcart.ShoppingCartEvents.getCartObject(request, locale, currencyUom);
