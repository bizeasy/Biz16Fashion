...
public class ShoppingCartEvents {
    ...
    {
        ShoppingCart sc = org.apache.ofbiz.order.shoppingcart.ShoppingCartEvents.getCartObject(request);
        ...
        if (sc.items().size() > 0) 
        {
            ...
            Map paymentMethodInfo = new HashMap<>();
    ...
    public static String resetPaymentMethod(HttpServletRequest request, HttpServletResponse response) 
    {
        ...
        if(UtilValidate.isEmpty(sc.items())) 
        {
            ...
            org.apache.ofbiz.order.shoppingcart.ShoppingCartEvents.clearCart(request, response);
    ...
    public static String addMultiItemsToCart(HttpServletRequest request, HttpServletResponse response) 
    {
        ...
        Delegator delegator = (Delegator) request.getAttribute("delegator");
        LocalDispatcher dispatcher = (LocalDispatcher) request.getAttribute("dispatcher");
        ShoppingCart shoppingCart = org.apache.ofbiz.order.shoppingcart.ShoppingCartEvents.getCartObject(request);
    ...
    public static String resetMultiShipGroups(HttpServletRequest request, HttpServletResponse response)
    {
        ...
        if (shoppingCart.items().size() > 0)
    	{
        	...
        	while (cartItemIter.hasNext())
    		{
        		...
        		List<String> lOrderItemAttributeDel = new LinkedList<String>();
				Map<String, String> mOrderItemAttributeAdd=new HashMap<String, String>();
        ...
        if (shoppingCart.items().size() > 0)
    	{
        	...
        	while (cartItemIter.hasNext())
    		{
        		...
        		if (UtilValidate.isNotEmpty(orderItemAttributesMap))
        		{
            		...
            		Map mShipItemQtyGroupIndexMap = new HashMap<>();
                	...
                	while (shipGroupQtyIter.hasNext())
                	{
                		...
                		Map mShipItemGroupInfoMap = new HashMap<>();
    ...
    public static String addLoyaltyPoints(HttpServletRequest request, HttpServletResponse response) 
    {
        ...
        LocalDispatcher dispatcher = (LocalDispatcher) request.getAttribute("dispatcher");
        Locale locale = UtilHttp.getLocale(request);
        ShoppingCart cart = org.apache.ofbiz.order.shoppingcart.ShoppingCartEvents.getCartObject(request);
    	...
    	{
    		orderAdjustmentAttributeList = new LinkedList<>();
    	}
    	else
    	{
    	...
    	Map loyaltyInfoMap = new HashMap<>();
    	try 
    	{            
    	...
    	Map loyaltyConversionMap = new HashMap<>();
    	try 
    	{            
    	...
    	if(UtilValidate.isNotEmpty(cart) && cart.size() > 0)
    	{
    		...
    		if ((loyaltyPointsCurrencyBD.compareTo(cartGrandTotal) > 0)) 
            {
            	...
            	serviceContext.put("checkoutLoyaltyConversion", checkoutLoyaltyConversionBD);
            	loyaltyConversionMap = new HashMap<>();
            	try 
            	{            
            ...
            Map<String, Object>  orderAdjustmentAttributeInfoMap = new HashMap<String, Object>();
    ...
    public static void updatePersonalizationMap(HttpServletRequest request, HttpServletResponse response) 
    {
        ...
        {
        	personalizationMap = new HashMap<>();
        }
    ...
    public static String addToCartCustom(HttpServletRequest request, HttpServletResponse response) 
    {
        ...
        String addToCartResult = org.apache.ofbiz.order.shoppingcart.ShoppingCartEvents.addToCart(request, response);
