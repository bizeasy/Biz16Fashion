...
public class CheckOutEvents {
        ...
        if(UtilValidate.isNotEmpty(cart))
        {
	    	try {
	            ...
	            for (int shipGroupIdx = 0; shipGroupIdx < shipGroups; shipGroupIdx++) 
	            {
	                ...
	                List promoLoyaltyAdjExps = new LinkedList<>();
    ...
    public static String setShipGroups(HttpServletRequest request, HttpServletResponse response) 
    {
        ...
        Map shippingContactMechMap = new HashMap<>();
        Map cartLineShippingContactMechQtyMap = new HashMap<>();
        Map cartLineShippingContactMechGiftMsgMap = new HashMap<>();
        Map cartLineQtyMap = new HashMap<>();
        Map cartLineProductInfoMap  = new HashMap<>();
        Map cartItemQtyMap = new HashMap<>();
        ...
        for (int i=0; i < iItemTotalQty;i++)
        {
        	...
        	Map giftMessageInfoMap = new HashMap<>();
            ...
            if (shoppingCart.items().size() != iNumberOfItems)
            {
	             ...
	             try
	             {
	            	...
	            	while (cartLineItemIter.hasNext())
	            	{
	                	...
	                	org.apache.ofbiz.osafe.events.ShoppingCartEvents.setProductFeaturesOnCart(shoppingCart,""+productInfoMap.get("productId"));
        ...
        {
    		Map<ShoppingCartItem, String> cartItemMessageCount = new HashMap<ShoppingCartItem, String>();
	    	...
	    	try 
	    	{
	            ...
	            for (int i = 0; i < lCartShipGroups.size(); i++) 
	            {
	                ...
	                if (UtilValidate.isNotEmpty(shipItems) && shipItems.size() > 1)
	                {
	                    ...
	                    Map<String, String> mShipGroupTempIndexShipMethods = new HashMap<String, String>();
	                    Map<String, String> mShipMethodKeys = new HashMap<String, String>();
