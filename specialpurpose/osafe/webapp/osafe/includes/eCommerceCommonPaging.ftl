<#if (pagingListSize?has_content && pagingListSize > 0)>
   <#assign showAll = 1/>
   <#assign paramMap = Static["org.apache.ofbiz.base.util.UtilHttp"].getParameterMap(request)!""/>
      <#if paramMap?has_content>
           <#assign previousParams = Static["org.apache.ofbiz.base.util.UtilHttp"].urlEncodeArgs(paramMap)/>
      <#else>
         <#assign previousParams = ""/>
      </#if>
      <#if previousParams?has_content>
         <#assign previousParams = Static["org.apache.ofbiz.base.util.UtilHttp"].stripNamedParamsFromQueryString(previousParams,Static["org.apache.ofbiz.base.util.UtilMisc"].toSet("viewSize","viewIndex","showPagesLink"))/>
      </#if>
   ...
   <ul class="pageingControls">
      ...
      <#if (viewIndex > 1)>
      	<li class="pagingBtn first"><a href="${request.org.apache.ofbiz.getRequestURI()!""}<#if previousParams?has_content>?${previousParams}<#if !previousParams.contains('&')>&</#if><#else>?</#if>viewSize=${showPages!}&viewIndex=${showAll!}">${uiLabelMap.FirstLabel}</a></li>
