...
<#macro facetLine facet facetType refinementValueName refinementValue multiFacetRefinedExist multiFacetRefined multiFacetInitialType facetGroupParamList allSelected showItemCount>
    ...
    <#if FACET_VALUE_STYLE == "CHECKBOX">
        ...
        <#assign removeUrl = Static["org.apache.ofbiz.osafe.control.SeoUrlHelper"].makeSeoFriendlyUrl(request,'${removeUrl}')/>
    ...
    <#elseif FACET_VALUE_STYLE == "DROPDOWN">  
        ...
        <#if newFilterGroupValue?has_content>
	        	<#assign newUrl = newUrl+"&filterGroup=${newFilterGroupValue}"/>
	          	<#assign newUrl = Static["org.apache.ofbiz.osafe.control.SeoUrlHelper"].makeSeoFriendlyUrl(request,'${newUrl}')/>
...
<#macro navBar parentCategory category levelUrl levelValue listIndex listSize>
  ...
  <#local macroLevelUrl = Static["org.apache.ofbiz.osafe.control.SeoUrlHelper"].makeSeoFriendlyUrl(request,'${macroLevelUrl}?productCategoryId=${category.productCategoryId}')>
...
<#macro renderCrumb category>
      ...
      <li>
        ...
        <#if (curCategoryId?exists && curCategoryId == category.productCategoryId) && !facetGroups?has_content && !product_id?has_content>
           <span>${catName}</span>
        <#else>
           <#assign catalogFriendlyUrl = Static["org.apache.ofbiz.osafe.control.SeoUrlHelper"].makeSeoFriendlyUrl(request,'${categoryUrl}?productCategoryId=${category.productCategoryId!""}')/>
...
<#macro renderFacetCrumb facet listSize index>
    ...
    <#if productCategoryIdFacet?has_content && productCategoryIdFacet=="Y">
        <#if facetPart1?has_content && facetPart1=="productCategoryId">
            <#assign productCategory = delegator.findOne("ProductCategory", Static["org.apache.ofbiz.base.util.UtilMisc"].toMap("productCategoryId", facetPart2), true)>
      ...
      <li>
           ...
           <#if index==listSize>
              <#if facetPart2Desc?has_content>
                  <#assign facetPart2Desc = Static["org.apache.ofbiz.base.util.StringUtil"].wrapString(facetPart2Desc)!/>
              </#if>
