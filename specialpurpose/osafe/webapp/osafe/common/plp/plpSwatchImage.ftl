<#if plpProductSelectableFeatureAndAppl?has_content>
 <#assign PRODUCT_MONEY_THRESHOLD = Static["org.apache.ofbiz.osafe.util.Util"].getProductStoreParm(request,"PRODUCT_MONEY_THRESHOLD")!"0"/>
 <#assign PRODUCT_PCT_THRESHOLD = Static["org.apache.ofbiz.osafe.util.Util"].getProductStoreParm(request,"PRODUCT_PCT_THRESHOLD")!"0"/>
  ...
  <div class="swatch">
    <#list plpProductSelectableFeatureAndAppl as productFeatureAppls>
      ...
      <#if productFeatureVariantId?has_content>
        <#assign variantProductUrl = Static["org.apache.ofbiz.osafe.control.SeoUrlHelper"].makeSeoFriendlyUrl(request, StringUtil.wrapString(plpPdpUrl) + "&productFeatureType=${productFeatureTypeId!}:${productFeatureDescription!}") />
