<#if currentProduct.isVirtual?if_exists?upper_case == "Y">
  <li class="${request.getAttribute("attributeClass")!}">
   <div class="pdpSelectableFeature">
	<#assign inStock = true />
	<#assign isSellable = Static["org.apache.ofbiz.product.product.ProductWorker"].isSellable(currentProduct?if_exists) />
	<#if !isSellable>
	 <#assign inStock=false/>
	      ...
	      <#if variantList?exists>
		      <#list variantList as variantProductAssocs>
		        ...
		        <#if productVariantProductMap.get(variantProductAssocProductId)?exists && productVariantProductMap.get(variantProductAssocProductId)?has_content >
                  ...
                  <#assign featureClass=  Static["org.apache.ofbiz.osafe.util.Util"].removeNonAlphaNumeric(featureDescListString)!""/> 
	  ...
	  <#else>
	    ...
	    <#if !currentProduct.virtualVariantMethodEnum?exists || currentProduct.virtualVariantMethodEnum == "VV_VARIANTTREE">
	      <#assign pdpSwatchImageHeight = Static["org.apache.ofbiz.osafe.util.Util"].getProductStoreParm(request,"IMG_SIZE_PDP_SWATCH_H")!""/>
	      <#assign pdpSwatchImageWidth = Static["org.apache.ofbiz.osafe.util.Util"].getProductStoreParm(request,"IMG_SIZE_PDP_SWATCH_W")!""/>
	          ...
	          <div class="selectableFeatures ${productFeatureTypeId}">
	            ...
	            <#if productFacetTooltipMap?exists && productFacetTooltipMap?has_content>
	              ...
	              <#if productFacetTooltip?has_content>
                    <#assign facetTooltipTxt = Static["org.apache.ofbiz.osafe.util.Util"].formatToolTipText("${productFacetTooltip}", "${productFacetTooltip?length}")/>
	            ...
	            <ul class="js_selectableFeature_${featureIdx}" id="LiFT${productFeatureTypeId}" name="LiFT${productFeatureTypeId}">
	              <#list productFeatureAndApplsSelects as productFeatureAndApplsSelect>
                    ...
                    <#assign featureClass=  Static["org.apache.ofbiz.osafe.util.Util"].removeNonAlphaNumeric(productFeatureDescription)!""/> 
	                ...
	                <#if PDP_FACET_GROUP_VARIANT_SWATCH?has_content && productFeatureTypeId.equalsIgnoreCase(PDP_FACET_GROUP_VARIANT_SWATCH)>
	                  ...
	                  <#if productFeatureSelectVariantId?has_content>
	                    <#if !alreadyShownProductFeatureId.contains(productFeatureSelectVariantId)>
	                      ...
	                      <#assign variantProductUrl = Static["org.apache.ofbiz.osafe.control.SeoUrlHelper"].makeSeoFriendlyUrl(request, "eCommerceProductDetail?productId=${productId!}&productCategoryId=${productCategoryId!}&productFeatureType=${productFeatureTypeId!}:${productFeatureDescription!}") />
