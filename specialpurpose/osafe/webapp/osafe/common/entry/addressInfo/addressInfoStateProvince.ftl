...
<#if !Static["org.apache.ofbiz.osafe.util.Util"].isProductStoreParmTrue(COUNTRY_MULTI!"")>
	<#assign selectedCountry = defaultCountryGeoMap.geoId/>
</#if>
        ...
        <select id="js_${fieldPurpose?if_exists}_STATE" name="${fieldPurpose?if_exists}_STATE" class="select ${fieldPurpose?if_exists}_COUNTRY">
            <#list countryList as country>
                <#if country.geoId == selectedCountry>
                  <#assign stateMap = dispatcher.runSync("getAssociatedStateList", Static["org.apache.ofbiz.base.util.UtilMisc"].toMap("countryGeoId", country.geoId, "userLogin", userLogin, "listOrderBy", "geoCode"))/>
                  <#assign stateList = stateMap.stateList />
                  <#-- assign stateList = Static["org.apache.ofbiz.common.CommonWorkers"].getAssociatedStateList(delegator, country.geoId) /-->
