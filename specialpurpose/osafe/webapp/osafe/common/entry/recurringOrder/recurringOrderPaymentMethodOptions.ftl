         ...
         <#if savedPaymentMethodValueMaps?has_content>
            <#list savedPaymentMethodValueMaps as savedPaymentMethodValueMap>
               ...
               <#if ("CREDIT_CARD" == savedPaymentMethod.paymentMethodTypeId) && (savedCreditCard?has_content)>
                 ...
                 <#if (cardExpireDate?has_content) && (Static["org.apache.ofbiz.base.util.UtilValidate"].isDateAfterToday(cardExpireDate)) && (cardNumber?has_content) && ((!alreadyShownSavedCreditCardList.contains(cardNumber+cardExpireDate)) || selectedSavedPayment == savedPaymentMethod.paymentMethodId)>
