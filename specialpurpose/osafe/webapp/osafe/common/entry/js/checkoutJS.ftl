...
<#assign allowCOD = Static["org.apache.ofbiz.osafe.util.Util"].isProductStoreParmTrue(request,"CHECKOUT_ALLOW_COD")/>
...
<#if formName?has_content>
    ...
    function setShippingMethod(selectedShippingOption, isOnLoad) 
    {
        ...
        if (jQuery('.js_onePageCheckoutOrderItemsSummary').length) 
        {
            ...
            {
                <#assign storeCC = Static["org.apache.ofbiz.osafe.util.Util"].isProductStoreParmTrue(request,"CHECKOUT_STORE_CC")/>
                <#assign storeCCReq = Static["org.apache.ofbiz.osafe.util.Util"].isProductStoreParmTrue(request,"CHECKOUT_STORE_CC_REQ")/>
