<script language="JavaScript" type="text/javascript">
 <#if currentProduct?exists>
  <#assign PDP_QTY_DEFAULT = Static["org.apache.ofbiz.osafe.util.Util"].getProductStoreParm(request,"PDP_QTY_DEFAULT")!/>
  <#if !PDP_QTY_DEFAULT?has_content || !(Static["org.apache.ofbiz.osafe.util.Util"].isNumber(PDP_QTY_DEFAULT))>
  ...
  jQuery(function() 
  {
    ...
    function replaceDetailImage(largeImageUrl, detailImageUrl) 
    {
        ...
        <#assign activeZoom = Static["org.apache.ofbiz.osafe.util.Util"].isProductStoreParmTrue(request,"PDP_IMG_ZOOM_ACTIVE_FLAG")/>
        <#if activeZoom>
            var mainImages = jQuery('#js_mainImageDiv').clone();
            jQuery(mainImages).find('img.js_productLargeImage').attr('id', 'js_mainImage');
        ...
        if (document.images['js_mainImage'] != null) 
        {
            ...
            <#assign IMG_SIZE_PDP_REG_W = Static["org.apache.ofbiz.osafe.util.Util"].getProductStoreParm(request,"IMG_SIZE_PDP_REG_W")!""/>
    ...
    {
        <#if Static["org.apache.ofbiz.osafe.util.Util"].isProductStoreParmTrue(request,"PDP_ALT_IMG_SCROLLER_ACTIVE")>
            ...
            {
                <#if Static["org.apache.ofbiz.osafe.util.Util"].isProductStoreParmTrue(request,"PDP_ALT_IMG_SCROLLER_VERTICAL")>
                    vertical: true,
                </#if>
  ...
  function updateImageText(idx) 
  {
		...
		<#if textPersonalization?has_content && textPersonalization == "TRUE">
			...
			{	
				...
				<#if textLinesNum?has_content && Static["org.apache.ofbiz.osafe.util.Util"].isNumber(textLinesNum) && (textLinesNum?number &gt; 0)>
					if(!jQuery('#js_imageTextArea').length)
					{
