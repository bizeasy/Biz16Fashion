...
<#assign FACET_VALUE_STYLE = Static["org.apache.ofbiz.osafe.util.Util"].getProductStoreParm(request,"FACET_VALUE_STYLE")!"NORMAL"/>
...
<#if facetTopProdCatContentTypeId?exists && facetTopProdCatContentTypeId?has_content>
    ...
    <#if facetTopContentId?exists >
        <#assign facetTopPlpEspotContent = delegator.findOne("Content", Static["org.apache.ofbiz.base.util.UtilMisc"].toMap("contentId", facetTopContentId), true) />
    </#if>
</#if>
...
<ul>
    ...
    <#assign facetShowItemCount = Static["org.apache.ofbiz.osafe.util.Util"].isProductStoreParmTrue(request,"FACET_SHOW_ITEM_CNT")/>
    ...
    <#assign SEARCH_FACET_GROUP_INCLUDE = Static["org.apache.ofbiz.osafe.util.Util"].getProductStoreParm(request,"SEARCH_FACET_GROUP_INCLUDE")!"">
    ...
    <#if loopFacetList?has_content>
        <#list loopFacetList as facetResult>
            ...
            <#if productFeatureCatGrpAppls?has_content>
                <#assign productFeatureCatGrpAppl = Static["org.apache.ofbiz.entity.util.EntityUtil"].getFirst(productFeatureCatGrpAppls) />
            ...
            <#if !facetMinValue?has_content>
                <#assign facetMinValue = Static["org.apache.ofbiz.osafe.util.Util"].getProductStoreParm(request,"FACET_VALUE_MIN")!0 />
            </#if>
            <#if !facetMaxValue?has_content>
                <#assign facetMaxValue = Static["org.apache.ofbiz.osafe.util.Util"].getProductStoreParm(request,"FACET_VALUE_MAX")!99 />
            </#if>
            <#if facetMinValue?has_content>
            ...
            <#if facet.type?has_content && facet.type == 'productCategoryId'>
                <#if parameters._CURRENT_VIEW_ == 'eCommerceCategoryList' || Static["org.apache.ofbiz.osafe.util.Util"].isProductStoreParmTrue(request,"FACET_CAT_ON_PLP")>
                    		...
                    		<#if FACET_VALUE_STYLE == "DROPDOWN">
                    			<select name="${facet.name?lower_case?replace(" ","_")}" id="${facet.name?lower_case?replace(" ","_")}" class="facetGroup facetGroup_${facet.type}" onchange="solrSearch(this, '', '', 'select')">
                    				<#list facet.refinementValues as refinementValue>
	                                    ...
	                                    <#assign productCategoryUrl = Static["org.apache.ofbiz.osafe.control.SeoUrlHelper"].makeSeoFriendlyUrl(request,'${refinementURL}')/>
            ...
            <#elseif facet.type?has_content && facet.type == 'price'>
                ...
                <#if filterGroupParamMap?has_content && filterGroupParamMap.get("PRICE")?has_content>
                    <#assign facetGroupParamList = Static["org.apache.ofbiz.base.util.UtilGenerics"].checkList(filterGroupParamMap.get("PRICE"))/>
                </#if>
            ...
            <#elseif facet.type?has_content && facet.type == 'customerRating'>
                ...
                <#if filterGroupParamMap?has_content && filterGroupParamMap.get("CUSTOMER_RATING")?has_content>
                    <#assign facetGroupParamList = Static["org.apache.ofbiz.base.util.UtilGenerics"].checkList(filterGroupParamMap.get("CUSTOMER_RATING"))/>
                </#if>
            ...
            <#else>
                ...
                <#if filterGroupParamMap?has_content && filterGroupParamMap.get(facet.type)?has_content>
                    <#assign facetGroupParamList = Static["org.apache.ofbiz.base.util.UtilGenerics"].checkList(filterGroupParamMap.get(facet.type))/>
                </#if>
				<#if facetMultiSelect>
					<#assign valueSize = facet.refinementValues.size()/>
				<#else>
                        ...
                        <li class="facetGroup_${facet.type}">
                            <h3 class="facetGroup <#if facetMinValue == 0>js_showHideFacetGroupLink<#if !facetGroupParamList?has_content> js_seeMoreFacetGroupLink<#else> js_seeLessFacetGroupLink</#if></#if>">${facet.name}
                                <#if productFeatureCatGrpAppl?has_content && productFeatureCatGrpAppl.facetTooltip?has_content>
                                    <#assign facetTooltipTxt = Static["org.apache.ofbiz.osafe.util.Util"].formatToolTipText("${productFeatureCatGrpAppl.facetTooltip}", "${productFeatureCatGrpAppl.facetTooltip?length}")/>
...
<#if facetEndProdCatContentTypeId?exists && facetEndProdCatContentTypeId?has_content>
    ...
    <#if facetEndContentId?exists >
        <#assign facetEndPlpEspotContent = delegator.findOne("Content", Static["org.apache.ofbiz.base.util.UtilMisc"].toMap("contentId", facetEndContentId), true) />
    </#if>
</#if>
