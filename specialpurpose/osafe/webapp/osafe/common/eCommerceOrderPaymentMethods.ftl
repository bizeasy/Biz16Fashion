...
<#if userLogin?has_content>
    ...
    <#assign partyProfileDefault = delegator.findOne("PartyProfileDefault", Static["org.apache.ofbiz.base.util.UtilMisc"].toMap("partyId", partyId, "productStoreId", productStore.productStoreId), true)?if_exists/>
</#if>

<#if shoppingCart?has_content && shoppingCart.getOrderAttribute("STORE_LOCATION")?has_content && !Static["org.apache.ofbiz.osafe.util.Util"].isProductStoreParmTrue(request,"CHECKOUT_STORE_CC")>
...
<#assign checkOutStoreCC = Static["org.apache.ofbiz.osafe.util.Util"].getProductStoreParm(request,"CHECKOUT_STORE_CC")!""/>
<#assign checkOutStoreCCReq = Static["org.apache.ofbiz.osafe.util.Util"].getProductStoreParm(request,"CHECKOUT_STORE_CC_REQ")!""/>
<#assign checkoutPaymentStyle = Static["org.apache.ofbiz.osafe.util.Util"].getProductStoreParm(request,"CHECKOUT_PAYMENT_STYLE")!"LIST"/>
...
<#if shoppingCart?has_content && shoppingCart.getOrderAttribute("STORE_LOCATION")?has_content>
	<#if !Static["org.apache.ofbiz.osafe.util.Util"].isProductStoreParmTrue(request,"CHECKOUT_STORE_CC_REQ") && Static["org.apache.ofbiz.osafe.util.Util"].isProductStoreParmTrue(request,"CHECKOUT_STORE_CC")>
		...
		<div>
			...
			<div id="js_checkoutPaymentOptions" <#if ccAvailable == "false" || (remainingPayment <= 0)>style="display:none"</#if>>
	                       	...
	                       	<div class="entryField">
	                         	<select id="js_paymentOptionsDD" name="paymentOption">
	                                <option value="PAYOPT_EMPTY" <#if selectedPaymentOption == "PAYOPT_EMPTY"> selected</#if>>${uiLabelMap.SelectOneLabel}</option>
	            			  		<#if Static["org.apache.ofbiz.osafe.util.Util"].isProductStoreParmTrue(request,"CHECKOUT_ALLOW_CC")>
									...
									<#if Static["org.apache.ofbiz.osafe.util.Util"].isProductStoreParmTrue(request,"CHECKOUT_KEEP_PAYMENT_METHODS") && (userLogin?has_content) && !(userLogin.userLoginId == "anonymous")>
	          			      	   ...
	          			      	   <#if Static["org.apache.ofbiz.osafe.util.Util"].isProductStoreParmTrue(request,"CHECKOUT_ALLOW_EFT")>
						           ...
						           <#if Static["org.apache.ofbiz.osafe.util.Util"].isProductStoreParmTrue(request,"CHECKOUT_ALLOW_PAYPAL")>
	                            		<option value="PAYOPT_PAYPAL" <#if selectedPaymentOption == "PAYOPT_PAYPAL"> selected</#if>>${uiLabelMap.PaymentOptionPayPalLabel}</option>
	                               </#if>
						           <#if Static["org.apache.ofbiz.osafe.util.Util"].isProductStoreParmTrue(request,"CHECKOUT_ALLOW_EBS")>
	                            		<option value="PAYOPT_EBS" <#if selectedPaymentOption == "PAYOPT_EBS"> selected</#if>>${uiLabelMap.PaymentOptionEbsLabel}</option>
	                               </#if>
						           <#if Static["org.apache.ofbiz.osafe.util.Util"].isProductStoreParmTrue(request,"CHECKOUT_ALLOW_PAYNETZ")>
	                               ...
	                               <#if (Static["org.apache.ofbiz.osafe.util.Util"].isProductStoreParmTrue(request,"CHECKOUT_ALLOW_COD"))>					           
        	 ...
        	 <#else>
					...
					<#elseif Static["org.apache.ofbiz.osafe.util.Util"].isProductStoreParmTrue(request,"CHECKOUT_ALLOW_CC")>
						<#if Static["org.apache.ofbiz.osafe.util.Util"].isProductStoreParmTrue(request,"CHECKOUT_KEEP_PAYMENT_METHODS") && savedPaymentMethodValueMaps?has_content && (userLogin?has_content) && !(userLogin.userLoginId == "anonymous")>
							<#list savedPaymentMethodValueMaps as savedPaymentMethodValueMap>
								<#assign savedPaymentMethod = savedPaymentMethodValueMap.paymentMethod/>
								<#if "CREDIT_CARD" == savedPaymentMethod.paymentMethodTypeId>
									<#assign hasSavedCard= "Y"/>
					...
					<#elseif Static["org.apache.ofbiz.osafe.util.Util"].isProductStoreParmTrue(request,"CHECKOUT_ALLOW_EFT")>
						<#if Static["org.apache.ofbiz.osafe.util.Util"].isProductStoreParmTrue(request,"CHECKOUT_KEEP_PAYMENT_METHODS") && savedPaymentMethodEftValueMaps?has_content && (userLogin?has_content) && !(userLogin.userLoginId == "anonymous")>
							<#list savedPaymentMethodEftValueMaps as savedPaymentMethodEftValueMap>
								<#assign savedPaymentMethod =savedPaymentMethodEftValueMap.paymentMethod/>
								<#if "EFT_ACCOUNT" == savedPaymentMethod.paymentMethodTypeId>
									<#assign hasSavedEftAccount = "Y"/>
					...
					<#-- END CREDIT CARD -->
			     	...
			     	<#-- EFT -->
			      	...
			      	<#-- COD-->
			      	<div class="container paymentOption js_codOptions" <#if (shoppingCart?has_content && shoppingCart.getOrderAttribute("STORE_LOCATION")?has_content) || !(Static["org.apache.ofbiz.osafe.util.Util"].isProductStoreParmTrue(request,"CHECKOUT_ALLOW_COD"))>style="display:none"</#if>>
