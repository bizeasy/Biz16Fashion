...
public class OrderLookupServices {
    ...
    public static Map findOrders(DispatchContext dctx, Map context) {
        ...
        List fieldsToSelect = new LinkedList<>();
...
/*        //Download Status
        ...
        if (UtilValidate.isNotEmpty(userLoginId) && UtilValidate.isEmpty(partyId)) {
            ...
            try {
                ul = EntityQuery.use(delegator).from("UserLogin").where(UtilMisc.toMap("userLoginId", userLoginId)).cache().queryOne();
        ...
        List completeOrderList = new LinkedList<>();
        int orderCount = 0;
