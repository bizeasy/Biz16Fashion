    ...
    public static Map<String, Object> findParty(DispatchContext dctx, Map<String, ? extends Object> context) {
        ...
        try {
            ...
            if (UtilValidate.isNotEmpty(roleTypeId)) {
                GenericValue currentRole = EntityQuery.use(delegator).from("RoleType").where(UtilMisc.toMap("roleTypeId", roleTypeId)).cache().queryOne();
        ...
        try {
            ...
            if (UtilValidate.isNotEmpty(partyTypeId)) {
                GenericValue currentPartyType = EntityQuery.use(delegator).from("PartyType").where(UtilMisc.toMap("partyTypeId", partyTypeId)).cache().queryOne();
        ...
        try {
            ...
            if (UtilValidate.isNotEmpty(stateProvinceGeoId)) {
                GenericValue currentStateGeo = EntityQuery.use(delegator).from("Geo").where(UtilMisc.toMap("geoId", stateProvinceGeoId)).cache().queryOne();
        ...
        List completePartyList = new LinkedList<>();
...
//                    findOpts.setMaxRows(highIndex);
        ...
        if (partyList == null) partyList = new LinkedList<>();
