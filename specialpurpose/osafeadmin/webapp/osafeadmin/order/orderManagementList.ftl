            ...
            <#list resultList as orderHeader>
				...
				<#else>
	                ...
	                <#if orderEmailContactMechs?has_content>
	                  <#assign orderEmail = Static["org.apache.ofbiz.entity.util.EntityUtil"].getFirst(orderEmailContactMechs)/>
                ...
                <tr class="dataRow <#if rowClass == "2">even<#else>odd</#if>">
                    ...
                    <td class="dateCol <#if !orderHeader_has_next>lastRow</#if>"><#--${orderHeader.getTimestamp("orderDate")?string("'"+preferredDateFormat+"'")}-->${Static["org.apache.ofbiz.base.util.UtilDateTime"].toDateString(orderHeader.getTimestamp("orderDate"),preferredDateFormat)}</td>
                    ...
                    <td class="nameCol <#if !orderHeader_has_next>lastRow</#if>">
                        <#if displayParty?has_content>
                            <#assign displayPartyNameResult = dispatcher.runSync("getPartyNameForDate", Static["org.apache.ofbiz.base.util.UtilMisc"].toMap("partyId", displayParty.partyId, "lastNameFirst","Y", "compareDate", orderHeader.orderDate, "userLogin", userLogin))/>
