  ...
  <tbody>
    ...
    <#list resultList as variantProduct>
      ...
      <tr class="dataRow <#if rowClass?if_exists == "2">even<#else>odd</#if>">
        ...
        <td class="dateCol <#if !variantProduct_has_next?if_exists>lastRow</#if>">${Static["org.apache.ofbiz.base.util.UtilDateTime"].toDateString(variantProdDetail.introductionDate,preferredDateFormat)!""}<#--${(variantProdDetail.introductionDate?string("'"+preferredDateFormat+"'"))!""}--></td>
        <td class="dateCol <#if !variantProduct_has_next?if_exists>lastRow</#if>">${Static["org.apache.ofbiz.base.util.UtilDateTime"].toDateString(variantProdDetail.salesDiscontinuationDate,preferredDateFormat)!""}<#--${(variantProdDetail.salesDiscontinuationDate?string("'"+preferredDateFormat+"'"))!""}--></td>
        ...
        <#if productVariantListPrice?has_content>
	    	...
	    	<#assign variantProductPriceInfo = variantProductPriceInfo + "<p>"+Static["org.apache.ofbiz.base.util.UtilProperties"].getMessage("OSafeAdminUiLabels","VariantOverridePriceInfo",pricesMap, locale )+"</p>">
	    <#elseif productListPrice?has_content>
	    	<#assign listPrice = productListPrice.price!"" />
	    	<#assign pricesMap = Static["org.apache.ofbiz.base.util.UtilMisc"].toMap("productListPrice", globalContext.currencySymbol+""+listPrice!"")>
	    	<#assign variantProductPriceInfo = variantProductPriceInfo + "<p>"+Static["org.apache.ofbiz.base.util.UtilProperties"].getMessage("OSafeAdminUiLabels","VirtualListPriceInfo",pricesMap, locale )+"</p>">
	  	</#if>
        ...
        <#if productVariantSalePrice?has_content>
	    	...
	    	<#assign variantProductPriceInfo = variantProductPriceInfo + "<p>"+Static["org.apache.ofbiz.base.util.UtilProperties"].getMessage("OSafeAdminUiLabels","VariantOverrideDefaultPriceInfo",pricesMap, locale )+"</p>">
	    <#elseif productDefaultPrice?has_content>
	    	<#assign defaultPrice = productDefaultPrice.price!"" />
	    	<#assign pricesMap = Static["org.apache.ofbiz.base.util.UtilMisc"].toMap("virtualSalePrice", globalContext.currencySymbol+""+defaultPrice)>
	    	<#assign variantProductPriceInfo = variantProductPriceInfo + "<p>"+Static["org.apache.ofbiz.base.util.UtilProperties"].getMessage("OSafeAdminUiLabels","VirtualDefaultPriceInfo",pricesMap, locale )+"</p>">
	  	</#if>
        <td class="dollarCol <#if !variantProduct_has_next?if_exists>lastRow</#if>">
