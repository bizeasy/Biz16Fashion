              ...
              <#assign hasNext = job_has_next/>
                <tr class="dataRow <#if rowClass == "2">even<#else>odd</#if>">                
                    ...
                    <#if recurrenceRule?exists && recurrenceRule?has_content>
                    	...
                    	<#assign rulesMap = Static["org.apache.ofbiz.base.util.UtilMisc"].toMap("intervalNumber", recurrenceRule.intervalNumber, "timeUnit", timeUnit, "runCountNumber", runCountNumber, "serviceNamefromList", job.serviceName!)>
                    	<#assign recurrenceRuleInfoString = recurrenceRuleInfoString + "<p>"+Static["org.apache.ofbiz.base.util.UtilProperties"].getMessage("OSafeAdminUiLabels","ServiceNameInfo",rulesMap, locale )+"</p>">
                    	<#assign recurrenceRuleInfoString = recurrenceRuleInfoString + "<p>"+Static["org.apache.ofbiz.base.util.UtilProperties"].getMessage("OSafeAdminUiLabels","RecurrenceRuleRepeatInfo",rulesMap, locale )+"</p>">
                    	<#assign recurrenceRuleInfoString = recurrenceRuleInfoString + "<p>"+Static["org.apache.ofbiz.base.util.UtilProperties"].getMessage("OSafeAdminUiLabels","RecurrenceRuleCountInfo",rulesMap, locale )+"</p>">
                    <#else>
			        ...
			        <#assign recurrenceRuleInfoString = "" >
                    <td class="dateCol <#if !hasNext>lastRow</#if>"><#if job.runTime?exists && job.runTime?has_content>${(Static["org.apache.ofbiz.osafe.util.OsafeAdminUtil"].convertDateTimeFormat(job.runTime, preferredDateTimeFormat).toLowerCase())!"N/A"}</#if></td>
                    <td class="dateCol <#if !hasNext>lastRow</#if>"><#if job.startDateTime?exists && job.startDateTime?has_content>${(Static["org.apache.ofbiz.osafe.util.OsafeAdminUtil"].convertDateTimeFormat(job.startDateTime, preferredDateTimeFormat).toLowerCase())!"N/A"}</#if></td>
                    <td class="dateCol <#if !hasNext>lastRow</#if>"><#if job.finishDateTime?exists && job.finishDateTime?has_content>${(Static["org.apache.ofbiz.osafe.util.OsafeAdminUtil"].convertDateTimeFormat(job.finishDateTime, preferredDateTimeFormat).toLowerCase())!"N/A"}</#if></td>
                    <td class="dateCol <#if !hasNext>lastRow</#if>"><#if job.cancelDateTime?exists && job.cancelDateTime?has_content>${(Static["org.apache.ofbiz.osafe.util.OsafeAdminUtil"].convertDateTimeFormat(job.cancelDateTime, preferredDateTimeFormat).toLowerCase())!"N/A"}</#if></td>
                    <#assign deleteScheduledJobConfirmListText = Static["org.apache.ofbiz.base.util.UtilProperties"].getMessage("OSafeAdminUiLabels", "DeleteScheduledJobConfirmListText", Static["org.apache.ofbiz.base.util.UtilMisc"].toMap("jobId", "${job.jobId!}"), locale)/>
