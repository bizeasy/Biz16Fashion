  ...
  <#assign prodCatMembershipCount = categoryMembers.size()!0/>
    ...
    <#if levelValue?has_content && levelValue="1">
      <tr class="dataRow ${levelClass} ${itemIndexClass} <#if rowClass == "2">even<#else>odd</#if>">
        ...
        <td class="dateCol"><#--${(category.fromDate?string("'"+preferredDateFormat+"'"))!""}-->${Static["org.apache.ofbiz.base.util.UtilDateTime"].toDateString(category.fromDate,preferredDateFormat)}</td>
        <td class="dateCol"><#--${(category.thruDate?string("'"+preferredDateFormat+"'"))!""}-->${Static["org.apache.ofbiz.base.util.UtilDateTime"].toDateString(category.thruDate,preferredDateFormat)}</td>
        <td class="actionCol">
