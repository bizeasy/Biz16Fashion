<#assign requestUri =  Static["org.apache.ofbiz.base.util.UtilHttp"].getRequestUriFromTarget(request.getRequestURL())/> 
<div id="siteInfo">
   <div id="welcome">
               ...
               <div class="contextInfoBox" style="display:none">
                   <#if adminContext?exists>
                       <#if adminContext.CONTEXT_PARTY_ID?has_content>
                           <#assign partyName = Static["org.apache.ofbiz.party.party.PartyHelper"].getPartyName(delegator, adminContext.CONTEXT_PARTY_ID?if_exists, false)?if_exists>
                       </#if>
                       ...
                       <#if adminContext.CONTEXT_ORDER_ID?has_content>
                           ...
                           <#if orderHeader?has_content>
                               <#assign orh = Static["org.apache.ofbiz.order.order.OrderReadHelper"].getHelper(orderHeader?if_exists)>
                               ...
                               <#if totalOrderItemsQuantity?has_content && totalOrderItemsQuantity &gt; 1>
                                   <#assign orderInfo = Static["org.apache.ofbiz.base.util.UtilProperties"].getMessage("OSafeAdminUiLabels", "ContextOrderItemsInfo", Static["org.apache.ofbiz.base.util.UtilMisc"].toList("${totalOrderItemsQuantity!}", "${globalContext.currencySymbol!}${grandTotal!}"), locale)/>
                               <#else>
                                   <#assign orderInfo = Static["org.apache.ofbiz.base.util.UtilProperties"].getMessage("OSafeAdminUiLabels", "ContextOrderItemInfo", Static["org.apache.ofbiz.base.util.UtilMisc"].toList("${totalOrderItemsQuantity!}", "${globalContext.currencySymbol!}${grandTotal!}"), locale)/>
                               </#if>
                           </#if>
                       </#if>
                       <div class=contextInfoRow>
